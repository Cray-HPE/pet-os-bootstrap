...
if [ ! -f /usr/bin/check-default-route.sh ]; then
  ...
  if [ "\$rc" -ne 0 ]; then
    ...
    ip route add default via 10.103.16.1
...
sed -i 's/vlan002/eth0/g' /srv/cray/scripts/metal/lib.sh
...
sed -i 's/bond0.nmn0/eth0/g' /srv/cray/scripts/metal/lib.sh
...
useradd -m -d /home/cdelatte -s /bin/bash -p 'REDACTED' cdelatte
sed -i -e '/root ALL=(ALL) ALL/a\' -e 'cdelatte ALL=(ALL) NOPASSWD: ALL' /etc/sudoers
